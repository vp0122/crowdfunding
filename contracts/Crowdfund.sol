...
contract CrowdFund {
    ...
    modifier onlyID(uint256 id) {
        require(id <= count, "Index out of count");
        _;
    }
    ...
    function cancel(uint _id) external onlyID(_id) {
    ...
    function pledge(uint _id, uint _amount) external onlyID(_id) {
    ...
    function unPledge(uint _id, uint _amount) external onlyID(_id) {
        Campaign storage campaign = campaigns[_id];
        require(
            block.timestamp >= campaign.startAt,
            "Campaign has not Started yet"
    ...
    function claim(uint _id) external onlyID(_id) {
        Campaign storage campaign = campaigns[_id];
        require(
            campaign.creator == msg.sender,
            "You did not create this Campaign"
    ...
    function refund(uint _id) external onlyID(_id) {
        Campaign memory campaign = campaigns[_id];
    ...
    function getPledgeAmount(
        uint _id
    ) external view onlyID(_id) returns (uint256) {
